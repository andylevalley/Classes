function [rHill,vHill] = eci2hill(rVC,vVC,rSat,vSat)
%% Purpose:
...
rTgtMag = sqrt(sum(rVC.^2,1));
rChaseMag = sqrt(sum(rSat.^2,1));
vTgtMag = sqrt(sum(vVC.^2,1));
...
r_Chase_RSW = matrixMultiply(RSW,rSat);
v_Chase_RSW = matrixMultiply(RSW,vSat);
